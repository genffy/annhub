...
export class LogseqClient {
  ...
  private async call<T = any>(method: string, ...args: any[]): Promise<T> {
    ...
    if (
      ...
      const errorDetail = data && typeof data === 'object' && 'error' in data ? (data as any).error : 'operation returned null or success=false'
  ...
  async createPage(
    ...
    return this.call<LogseqPage | null>('logseq.Editor.createPage', pageName, properties ?? {}, opts)
  }
  ...
  async appendBlockInPage(pageName: string, content: string, options?: { properties?: Record<string, any> }): Promise<LogseqBlock | null> {
    return this.call<LogseqBlock | null>('logseq.Editor.appendBlockInPage', pageName, content, options ?? {})
  }

  async insertBlock(srcBlockUUID: string, content: string, options?: { before?: boolean; sibling?: boolean; properties?: Record<string, any> }): Promise<LogseqBlock | null> {
    return this.call<LogseqBlock | null>('logseq.Editor.insertBlock', srcBlockUUID, content, options ?? {})
  }
   ...
   */
  async ensurePage(pageName: string, properties?: Record<string, any>): Promise<{ page: LogseqPage | null; created: boolean }> {
