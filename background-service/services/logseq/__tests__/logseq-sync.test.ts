...
describe('LogseqSyncService', () => {
  ...
  describe('syncHighlight', () => {
    ...
    it('should use page UUID for appendBlockInPage', async () => {
      ...
      expect(client.appendBlockInPage).toHaveBeenCalledWith('page-uuid-12345', expect.any(String), expect.any(Object))
    })
    ...
    it('should add user note as child block', async () => {
      ...
      expect(result.success).toBe(true)
      expect(client.insertBlock).toHaveBeenCalledWith('block-uuid', '💭 Important note!', { sibling: false })
    })
    ...
    it('should return error when appendBlockInPage returns null', async () => {
      ...
      vi.spyOn(client, 'appendBlockInPage').mockRejectedValue(new Error('Logseq API logseq.Editor.appendBlockInPage failed: operation returned null or success=false'))
  ...
  describe('syncAll', () => {
    ...
    it('should sync all active highlights', async () => {
      ...
      mockGetHighlights.mockResolvedValueOnce([makeHighlight({ id: 'hl1' }), makeHighlight({ id: 'hl2' }), makeHighlight({ id: 'hl3' })])
    ...
    it('should count failed syncs', async () => {
      ...
      mockGetHighlights.mockResolvedValueOnce([makeHighlight({ id: 'hl1' }), makeHighlight({ id: 'hl2' })])
