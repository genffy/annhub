...
const writeCategoryIdToHash = (categoryId: string): void => {
  ...
  const nextHash = params.toString() ? `${WORDS_ROUTE_PATH}?${params.toString()}` : WORDS_ROUTE_PATH
...
export default function WordsManagementPage() {
  ...
  const selectedCategory = useMemo(() => categories.find(item => item.id === selectedCategoryId), [categories, selectedCategoryId])
  const managerCategoryLabel =
    selectedCategoryId === ALL_CATEGORIES_ID ? i18n.t('options.words.allCategories') : selectedCategory?.name || i18n.t('options.words.noCategorySelected')
  ...
  const loadCategories = useCallback(async (): Promise<EudicCategory[]> => {
    ...
    try {
      ...
      setSelectedCategoryId(prev => {
        ...
        if (categoryIdFromHash && (categoryIdFromHash === ALL_CATEGORIES_ID || next.some(item => item.id === categoryIdFromHash))) {
  ...
  const loadWords = useCallback(
    async (categoryList: EudicCategory[] = categories, categoryIdOverride?: string) => {
      if (!canManageWords) {
        setRawWords([])
        return
      }
  ...
  const deleteWordsInCurrentScope = useCallback(
    async (words: string[]) => {
  ...
  const isAllCurrentPageSelected = currentPageWordKeys.length > 0 && currentPageWordKeys.every(key => selectedWordKeys.has(key))
  const isPartCurrentPageSelected = currentPageWordKeys.some(key => selectedWordKeys.has(key)) && !isAllCurrentPageSelected
  ...
  const paginationItems = useMemo(() => buildPaginationItems(managerPage, totalPages), [managerPage, totalPages])
  ...
  return (
    <Card>
      <PageHeader title={i18n.t('options.words.name')} description={i18n.t('options.words.description')} />
      <div className="vocab-settings">
        ...
        {!canManageWords && <div className="vocab-manager-empty">{i18n.t('options.words.configureToken')}</div>}
          ...
          <div className="vocab-manager">
            <aside className="vocab-manager-sidebar">
              ...
              <div className="vocab-manager-create">
                <input type="text" value={newCategoryName} onChange={e => setNewCategoryName(e.target.value)} placeholder={i18n.t('options.words.createCategoryPlaceholder')} />
                <button type="button" onClick={handleCreateCategory} disabled={isCategorySubmitting || !newCategoryName.trim()}>
            ...
            <div className="vocab-manager-main">
              <div className="vocab-manager-header">
                ...
                <div className="vocab-manager-header-actions">
                  <label className="vocab-manager-toolbar-item vocab-manager-toolbar-search">
                    ...
                    <input type="text" value={managerSearch} onChange={e => setManagerSearch(e.target.value)} placeholder={i18n.t('options.words.searchPlaceholder')} />
                  </label>
                  <label className="vocab-manager-toolbar-item">
                    <select value={managerSort} onChange={e => setManagerSort(e.target.value as SortMode)}>
                  ...
                  <label className="vocab-manager-toolbar-item">
                    ...
                    >
                      ...
                      <option value="delete-selected" disabled={selectedWordList.length === 0 || isBatchDeleting}>
                        {i18n.t('options.words.deleteSelected')}
                      </option>
                      <option value="rename-category" disabled={selectedCategoryId === ALL_CATEGORIES_ID || isCategorySubmitting}>
                        {i18n.t('options.words.renameCategory')}
                      </option>
                      <option value="delete-category" disabled={selectedCategoryId === ALL_CATEGORIES_ID || selectedCategory?.id === '0' || isCategorySubmitting}>
              ...
              <div className="vocab-manager-word-actions">
                ...
                />
                <button type="button" onClick={handleAddWord} disabled={isWordSubmitting || selectedCategoryId === ALL_CATEGORIES_ID || !newWord.trim()}>
                    ...
                    {isLoadingWords && (
                      <tr>
                        <td colSpan={7} className="vocab-manager-empty-row">
                          {i18n.t('options.words.loadingWords')}
                        </td>
                      </tr>
                    )}
                          ...
                          <tr key={`${word.word}-${word.add_time || absoluteIndex}`}>
                            ...
                            <td>
                              <button
                                ...
                                onClick={() => {
                                  void handleDeleteWord(word.word)
                                }}
              ...
              <div className="vocab-manager-pagination">
                <div className="vocab-manager-pagination-main">
                  <button type="button" className="page-btn" onClick={() => setManagerPage(prev => Math.max(1, prev - 1))} disabled={managerPage <= 1}>
                  ...
                  {paginationItems.map((item, idx) =>
                    item === 'ellipsis' ? (
                      ...
                      <button key={item} type="button" className={`page-btn ${managerPage === item ? 'active' : ''}`} onClick={() => setManagerPage(item)}>
                        {item}
                      </button>
                    ),
                  )}
                  <button type="button" className="page-btn" onClick={() => setManagerPage(prev => Math.min(totalPages, prev + 1))} disabled={managerPage >= totalPages}>
                ...
                <div className="vocab-manager-pagination-jump">
                  ...
                  />
                  <button type="button" className="page-jump-btn" onClick={handleJumpToPage}>
                    {i18n.t('options.words.goToPage')}
                  </button>
                </div>

                <div className="vocab-manager-pagination-size">
                  <select value={managerPageSize} onChange={e => setManagerPageSize(Number(e.target.value))}>
                    {PAGE_SIZE_OPTIONS.map(size => (
                      <option key={size} value={size}>
                        {i18n.t('options.words.pageSize', [size])}
                      </option>
                    ))}
                  </select>
                </div>
