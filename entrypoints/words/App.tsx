  ...
  useEffect(() => {
    ...
    const listener = (changes: { [key: string]: chrome.storage.StorageChange }) => {
      if (changes.vocabSnapshot) {
        setSnapshot((changes.vocabSnapshot.newValue as VocabSnapshot | undefined) ?? null)
      }
      if (changes.vocabSyncState) {
        setSyncState((changes.vocabSyncState.newValue as VocabSyncState | undefined) ?? null)
      }
    }
      ...
      <div className="sidebar-header">
        ...
        <div className="header-categories">
          <div className="sync-status">
            ...
            {syncState?.lastSyncStatus === 'error' && <span className="header-status-error"> — {syncState.lastError ?? 'Error'}</span>}
          </div>
        </div>
      </div>
          ...
          <div className="studylist-words">
            <div className="studylist-toolbar">
              ...
              <div className="toolbar-controls">
                <input type="text" className="search-input" placeholder="Search words..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
                <select className="studylist-select" value={sortMode} onChange={e => setSortMode(e.target.value as SortMode)}>
            ...
            <div className="studylist-words-content">
              {!snapshot && <div className="section-status">No vocabulary data yet. Configure sync in Options.</div>}

              {snapshot && wordEntries.length === 0 && <div className="section-status">{searchQuery ? 'No matching words found.' : 'No words in snapshot.'}</div>}
                    ...
                    return (
                      <article className="word-card" key={entry.word}>
                        <div className="word-header">
                          ...
                          <span className="word-proficiency">P{entry.proficiency}</span>
                          <button
                            type="button"
