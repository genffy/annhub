...
export class ClipService {
  ...
  private getContext(range: Range): { before: string; after: string } {
    ...
    const before = fullText.substring(Math.max(0, selStart - CONTEXT_LENGTH), selStart)
    const after = fullText.substring(selEnd, Math.min(fullText.length, selEnd + CONTEXT_LENGTH))
