  ...
  const loadHighlights = async () => {
    ...
    try {
      ...
      if (searchQuery.trim()) {
        ...
        filteredHighlights = allHighlights.filter(
          (h: HighlightRecord) =>
        ...
        <div className="highlight-list-header">
          ...
          <div className="header-controls">
            <div className="search-box">
              <input type="text" placeholder={`${i18n.t('search')}...`} value={searchQuery} onChange={e => handleSearch(e.target.value)} className="search-input" />
            </div>

            <div className="filter-box">
              <select value={statusFilter} onChange={e => handleStatusFilter(e.target.value as typeof statusFilter)} className="filter-select">
      ...
      <div className="highlight-list-content">
        ...
        {!loading && !error && highlights.length === 0 && (
          <div className="empty-state">
            ...
            <p>{i18n.t('noData')}</p>
            <p className="empty-description">{searchQuery ? i18n.t('noData') : i18n.t('noData')}</p>
          </div>
        )}
        ...
        {!loading && !error && highlights.length > 0 && (
          <div className="highlight-items">
            {highlights.map(highlight => (
              <div key={highlight.id} className="highlight-item" onClick={() => handleHighlightClick(highlight)}>
                  ...
                  <div className="highlight-status">
                    <span className={`status-badge status-${highlight.status}`}>
                      {highlight.status === 'active'
                        ? i18n.t('highlight.status.active')
                        : highlight.status === 'archived'
                          ? i18n.t('highlight.status.archived')
                          : i18n.t('highlight.status.deleted')}
                    </span>
                  </div>
                </div>
                    ...
                    <p className="highlight-context">
                      ...
                      <span className="context-text">
                        ...
                        <mark style={{ backgroundColor: highlight.color }}>
                          {highlight.originalText.length > 50 ? highlight.originalText.substring(0, 50) + '...' : highlight.originalText}
                        </mark>
                ...
                <div className="highlight-actions">
                  {highlight.metadata.sourceUrl && (
                    <span className="source-url-badge" title={highlight.metadata.sourceUrl}>
                      ...
                      {(() => {
                        try {
                          return new URL(highlight.metadata.sourceUrl).pathname
                        } catch {
                          return highlight.metadata.sourceUrl
                        }
                      })()}
                    </span>
                  )}
      ...
      {showPagination && pagination.totalPages > 1 && (
        <div className="highlight-pagination">
          <div className="pagination-info">
            ...
            <select value={pagination.pageSize} onChange={e => handlePageSizeChange(Number(e.target.value))} className="page-size-select">
          ...
          <div className="pagination-controls">
            <button onClick={() => handlePageChange(pagination.currentPage - 1)} disabled={pagination.currentPage === 1} className="pagination-button">
            ...
            <div className="pagination-numbers">
              {Array.from({ length: Math.min(5, pagination.totalPages) }, (_, i) => {
                const page = i + 1
                return (
                  <button key={page} onClick={() => handlePageChange(page)} className={`pagination-number ${pagination.currentPage === page ? 'active' : ''}`}>
                    {page}
                  </button>
                )
              })}
            ...
            <button onClick={() => handlePageChange(pagination.currentPage + 1)} disabled={pagination.currentPage === pagination.totalPages} className="pagination-button">
