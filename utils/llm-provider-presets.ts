...
export function getLlmProviderEndpoint(preset: LlmProviderPreset, endpointId?: string): LlmProviderPreset | LlmEndpointVariant {
  if (!endpointId) return preset
  return preset.endpointVariants?.find(endpoint => endpoint.id === endpointId) ?? preset
}

export function findLlmProviderEndpoint(providerPresetId?: string, endpointId?: string): (LlmProviderPreset | LlmEndpointVariant) | undefined {
