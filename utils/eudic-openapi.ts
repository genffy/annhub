...
async function eudicRequest<T>(token: string, path: string, options: EudicRequestOptions = {}): Promise<EudicApiResponse<T> | T> {
...
export async function fetchWords(token: string, categoryId: string, language = 'en', page = 0, pageSize = EUDIC_MAX_PAGE_SIZE): Promise<{ words: EudicWord[]; hasMore: boolean }> {
...
export async function fetchAllWords(token: string, categoryIds: string[], language = 'en'): Promise<EudicWord[]> {
